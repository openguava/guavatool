...
import io.github.openguava.guavatool.core.util.ArrayUtils;
...
public class Assert {
	 ...
	 * @return 经过检查后的值
	 */
	public static double checkBetween(double value, double min, double max, final String errorMsgTemplate, final Object... params) {
		return checkBetween(value, min, max, new FuncR<IllegalArgumentException>() {
			@Override
			public IllegalArgumentException call() {
				return new IllegalArgumentException(StringUtils.format(errorMsgTemplate, params));
			}
		});
	}
	 ...
	 * @since 5.7.15
	 */
	public static <X extends Throwable> double checkBetween(double value, double min, double max, FuncR<? extends X> errorSupplier) throws X {
		if (value < min || value > max) {
			throw errorSupplier.call();
		}
